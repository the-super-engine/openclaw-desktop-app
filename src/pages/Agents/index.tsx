...
function AgentSettingsModal({
  ...
  useEffect(() => {
    const loadConfigured = async () => {
      try {
        const result = await invokeIpc('channel:listConfigured') as { success: boolean; channels?: string[] };
  ...
  const handleChannelAssigned = async (channelType: ChannelType) => {
    try {
  ...
  const availableToAssign = configuredTypes.filter((t) => !agent.channelTypes.includes(t));
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
        ...
        <CardContent className="space-y-6 pt-4 overflow-y-auto flex-1 p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <Button
                onClick={() => setShowAssignChannelModal(true)}
      ...
      {showAssignChannelModal && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
          <Card className="w-full max-w-md rounded-3xl border-0 shadow-2xl bg-[#f3f1e9] dark:bg-[#1a1a19] overflow-hidden">
            <CardHeader className="flex flex-row items-start justify-between pb-2">
              <div>
                <CardTitle className="text-xl font-serif font-normal tracking-tight">
                  {t('settingsDialog.addChannel')}
                </CardTitle>
                <CardDescription className="text-[14px] mt-1 text-foreground/70">
                  {availableToAssign.length > 0
                    ? t('settingsDialog.channelsDescription')
                    : t('settingsDialog.noChannelsToAssign')}
                </CardDescription>
              </div>
              <Button variant="ghost" size="icon" onClick={() => setShowAssignChannelModal(false)}>
                <X className="h-4 w-4" />
              </Button>
            </CardHeader>
            <CardContent className="p-6">
              {availableToAssign.length > 0 ? (
                <div className="space-y-2">
                  {availableToAssign.map((channelType) => (
                    <button
                      key={channelType}
                      onClick={() => void handleChannelAssigned(channelType as ChannelType)}
                    ...
                    >
                      <div className="h-10 w-10 flex items-center justify-center rounded-full bg-black/5 dark:bg-white/5">
                        <ChannelLogo type={channelType as ChannelType} />
                      </div>
                      <span className="font-medium">{CHANNEL_NAMES[channelType as ChannelType] || channelType}</span>
                    </button>
                  ))}
                </div>
              ) : (
                <p className="text-muted-foreground text-sm">
                  {t('settingsDialog.configureChannelsFirst')}
                </p>
              )}
            </CardContent>
          </Card>
